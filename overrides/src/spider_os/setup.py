...
class SetupStore:
    ...
    def write(self, values: dict[str, Any]) -> dict[str, Any]:
        ...
        # Setup is not decorative: choosing a workspace applies it immediately.
        # Later Web Assembly runs preserve the active runtime mode instead of
        # repeatedly forcing the configured default over deliberate user changes.
...
<div class='card'><h2>Webbie</h2><label><input type='checkbox' name='voice_enabled' {checked('voice_enabled')}> Voice presence enabled</label><label><input type='checkbox' name='proactive' {checked('proactive_level')}> Very proactive assistance</label><small>Wake phrases: <code>{html.escape(', '.join(DEFAULT_WAKE_PHRASES))}</code>. Wake-word processing is designed to remain local.</small></div>
